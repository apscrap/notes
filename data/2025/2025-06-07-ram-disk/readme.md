...
>
> **Radeon_RAMDisk_4_4_0_RC36.msi**: `D792733F0EBC8C30D7F6778C9F0276CD5FBDE7419212D0B338744B0311D38970`.
...
## Симлинки (символические ссылки)

Множество программ используют системную папку `%APPDATA%` (`C:\Users\user_name\AppData\Roaming`) для хранения пользовательских данных. Часть программ даже не пытается поинтересоваться у пользователя, согласен ли он вообще с таким положением вещей. Например, этим постоянно грешат игры, сохранения которых могут занимать десятки и сотни метров.

Решение очень незатейливое: создать символическую ссылку на пользовательскую папку, чтобы физически файлы находились на RAM-диске, но система считала, что они в соответствующей папке `%APPDATA%`. Создадим такую ссылку на примере одной из гениальнейших игр современности — **Noita**.

Пользовательские данные хранятся по адресу `C:\Users\user_name\AppData\LocalLow\Nolla_Games_Noita`, а мы желаем разместить их по адресу `R:\Nolla_Games_Noita`.

Переносим папку `Nolla_Games_Noita` в новое место, после чего в консоли **cmd** прописываем команду `mklink`:

`mklink /J "C:\Users\user_name\AppData\LocalLow\Nolla_Games_Noita" "R:\Nolla_Games_Noita"`

Готово!

Понятное дело, состояние папки нужно будет бекапить и восстанавливать перед запуском игры, но оно того стоит. В той же нойте сейвы в лейтгейме могут и под гиг занимать.

Удаление симлинка производится командой `rmdir`:

`rmdir "C:\Users\user_name\AppData\Roaming\Noita"`
<br><br>
